...
db.stores = new Datastore({ filename: './database/stores.db' });
...
app.get('/remove_old_logs', function (req, res) {
    db.orderlogsdb.remove({ timestamp: { $lt: new Date().getTime() - 604800000 } }, { multi: true }, function (e, n) { // Callback is optional
        //res.send({ err: e })
        res.send({ msg: `${n} logs removed.`, error: e })
    })
});
...
app.post('/setstoredata', function (req, res) {
    ...
    db.stores.remove({}, { multi: true }, function (err, numRemoved) {
        db.stores.insert(req.body.stores, function (err, newDoc) {   // Callback is optional
            console.log("preference", i)
            i++
            if (i == 11)
                res.send(obj)
