...
export class MenuLeftComponent implements OnInit {
  ...
  keyEvent(event: KeyboardEvent) {
    let spl_keys = ['ArrowDown', 'ArrowUp', 'Enter', 'F10']
    if (spl_keys.includes(event.key))
      event.preventDefault()
    console.log(event.key)

    let bp_modal = document.getElementById("LeftList")
    console.log(bp_modal)
    if (event.key == "ArrowDown") {
      if (bp_modal) {
        let index = this.menuDataActivated.findIndex(x => x.focus == true)
        if (index < (this.menuDataActivated.length - 1)) {
          this.menuDataActivated[index].focus = false
          this.menuDataActivated[index + 1].focus = true
        }
      }
    ...
    console.log(this.menuDataActivated.length, "focus index: " + this.menuDataActivated.findIndex(x => x.focus == true))
